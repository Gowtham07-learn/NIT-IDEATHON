...
st.markdown("### Predictive Analytics • AI Advisory")
...
with st.expander("🛠️ Fault Injection Panel", expanded=True):
    cols = st.columns(3)
    ...
    with card_cols[i]:
        with st.container(border=True):
            ...
            if res["anomaly"]["anomaly_flag"] or ris_val > 50:
                ai_data = {
                    "name": svc,
                    "latency": f"{res['latest'].get('latency_ms', 0):.1f} ms",
                    "error_rate": f"{res['prediction'].get('predicted_error_rate', 0)*100:.0f}%",
                    "status": res['latest'].get('status', 200)
                }
                advice_text = get_dynamic_advice(svc, ai_data)
                if advice_text: st.success(advice_text)
                else: st.caption("🤖 AI is thinking...")
            else:
                st.write("✅ Normal operations.")

# Dependency Logic
st.divider()
st.subheader("🔗 Topology Status")
...
if bank_res.get('status') != 200:
    st.warning("⚠️ CRITICAL: Issuer Bank is failing.")
elif upi_res.get('status') != 200:
    st.warning("⚠️ CRITICAL: UPI PSP is failing.")
else:
